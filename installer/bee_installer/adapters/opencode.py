"""
OpenCode adapter - converts Bee format to OpenCode's format.

OpenCode (OhMyOpenCode) is a Claude Code-compatible AI agent platform.
It uses similar concepts to Bee/Claude Code with minor directory differences:
...
incompatible with Bee's file-based hooks. Bee hooks CANNOT be installed on OpenCode.
"""
...
from bee_installer.adapters.base import PlatformAdapter
...
class OpenCodeAdapter(PlatformAdapter):
    ...
    - Hooks: NOT SUPPORTED - OpenCode uses plugin-based hooks incompatible with Bee
    ...
    def transform_skill(self, skill_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee skill for OpenCode.

        OpenCode uses the same markdown + YAML frontmatter format as Bee/Claude Code.
    ...
    def transform_agent(self, agent_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee agent for OpenCode.
    ...
    def _strip_model_requirement_section(self, body: str) -> str:
        ...
        Bee agents include self-verification sections that check if the model
    ...
    def transform_command(self, command_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee command for OpenCode.
    ...
    def transform_hook(self, hook_content: str, metadata: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
        Transform a Bee hook for OpenCode.
        ...
        which is incompatible with Bee's file-based hooks.
        ...
        logger.warning(
            ...
            "Bee's file-based hooks. This hook will be skipped.",
            hook_name
        )
        return None
    ...
    def get_component_mapping(self) -> Dict[str, Dict[str, str]]:
        ...
        which are incompatible with Bee's file-based hooks.

        Returns:
            Mapping of Bee components to OpenCode directories (excludes hooks)
        """
        return {
            "agents": {
    ...
    def supports_component(self, component_type: str) -> bool:
        ...
        incompatible with Bee's file-based hooks.
    ...
    def get_terminology(self) -> Dict[str, str]:
        ...
        OpenCode uses the same terminology as Claude Code/bee for supported
        components. Note: hooks are intentionally excluded since OpenCode
        does not support Bee's file-based hooks.
    ...
    def is_native_format(self) -> bool:
        """
        Check if this platform uses Bee's native format.

        Returns:
            True - OpenCode uses a very similar format to Bee (near-native)
        """
    ...
    def requires_hooks_in_settings(self) -> bool:
        ...
        IMPORTANT: OpenCode does NOT support Bee's file-based hooks at all.
    ...
    def _transform_agent_frontmatter(self, frontmatter: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Convert Bee 'type' field to OpenCode 'mode' field
        if "type" in result:
    ...
    def _transform_permissions(self, permissions: Any) -> Optional[Dict[str, str]]:
        """
        Transform Bee permissions to OpenCode permission format.
        ...
        Args:
            permissions: Bee permission data (dict, list, or string)
        ...
        # If it's a list of permission names (Bee format)
    ...
    def merge_hooks_to_config(
        ...
        support Bee's file-based hooks. OpenCode uses a plugin-based hook system
