...
the required abstract methods for transforming Bee components.
"""
...
class PlatformAdapter(ABC):
    ...
    Each platform adapter handles the transformation of Bee components
    ...
    def transform_skill(self, skill_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee skill to the target platform format.

        Args:
            skill_content: The original skill content in Bee format (markdown with YAML frontmatter)
    ...
    def transform_agent(self, agent_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee agent to the target platform format.

        Args:
            agent_content: The original agent content in Bee format
    ...
    def transform_command(self, command_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee command to the target platform format.

        Args:
            command_content: The original command content in Bee format
    ...
    def get_component_mapping(self) -> Dict[str, Dict[str, str]]:
        """
        Get the mapping of Bee component types to platform-specific directories.
    ...
    def transform_hook(self, hook_content: str, metadata: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
        Transform a Bee hook to the target platform format.
        ...
        Returns:
            ...
            plugin-based hooks which are incompatible with Bee's file-based hooks).
        """
        return hook_content
    ...
    def get_terminology(self) -> Dict[str, str]:
        ...
        Returns:
            Dictionary mapping Bee terms to platform terms.
    ...
    def is_native_format(self) -> bool:
        ...
        Returns:
            True if the platform uses Bee format natively (no transformation needed)
        """
        return False
    ...
    def replace_terminology(self, content: str) -> str:
        """
        Replace Bee terminology with platform-specific terms.

        Args:
            content: Content with Bee terminology
        ...
        for bee_term, platform_term in terminology.items():
            if bee_term != platform_term:
                ...
                result = result.replace(bee_term, platform_term)
                result = result.replace(bee_term.title(), platform_term.title())
                result = result.replace(bee_term.upper(), platform_term.upper())
