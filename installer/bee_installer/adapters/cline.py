"""
Cline adapter - converts Bee format to Cline prompts.
...
class ClineAdapter(PlatformAdapter):
    ...
    def transform_skill(self, skill_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee skill to a Cline prompt.

        Converts the Bee skill format to Cline's prompt format, which uses
    ...
    def transform_agent(self, agent_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee agent to a Cline prompt.
    ...
    def transform_command(self, command_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee command to a Cline prompt.
    ...
    def get_component_mapping(self) -> Dict[str, Dict[str, str]]:
        ...
        Returns:
            Mapping of Bee components to Cline directories
        """
        return {
            "agents": {
    ...
    def get_terminology(self) -> Dict[str, str]:
        ...
        Returns:
            Mapping of Bee terms to Cline terms
        """
        return {
            "agent": "prompt",
            "skill": "prompt",
    ...
    def _transform_body_for_cline(self, body: str) -> str:
        ...
        # Transform Bee tool references to Cline prompt references
        result = re.sub(
            r'`bee:([^`]+)`',
    ...
    def _format_prompt_reference(self, name: str) -> str:
        """
        Format a Bee reference as a Cline prompt reference.

        Args:
            name: Bee component name

        Returns:
            Cline prompt reference format
        """
        # Convert kebab-case to path-like format
        # bee:code-reviewer -> code-reviewer
    ...
    def generate_prompt_index(self, prompts: List[Dict[str, str]]) -> str:
        ...
        """
        lines = [
            "# Bee Prompts for Cline",
            "",
            "This directory contains Bee skills, agents, and commands converted to Cline prompts.",
            "",
        ]
