"""
Cursor adapter - converts Bee format to Cursor native format.
...
from bee_installer.transformers.base import BaseTransformer, normalize_cursor_name
...
class CursorAdapter(PlatformAdapter):
    ...
    def transform_skill(self, skill_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee skill to Cursor skill format.

        Converts the Bee skill format to Cursor's skill format with YAML
    ...
    def transform_agent(self, agent_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee agent to Cursor agent format.
    ...
    def transform_command(self, command_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee command to Cursor command format.
    ...
    def get_component_mapping(self) -> Dict[str, Dict[str, str]]:
        ...
        Returns:
            Mapping of Bee components to Cursor directories
        """
        return {
            "agents": {
                "target_dir": "agents",
    ...
    def get_terminology(self) -> Dict[str, str]:
        ...
        Returns:
            Mapping of Bee terms to Cursor terms
        """
        return {
            "agent": "agent",
            "skill": "skill",
    ...
    def _transform_body_for_cursor(self, body: str) -> str:
        ...
        result = re.sub(r'`bee:[^`]+`', lambda m: self._transform_bee_reference(m.group(0)), result)

        # Normalize /bee: command references for all component types
        result = result.replace("/bee:", "/")

        return result

    def _transform_bee_reference(self, ref: str) -> str:
        """
        Transform a Bee tool reference to Cursor format.

        Args:
            ref: Bee reference like `bee:skill-name`

        Returns:
            Cursor-friendly reference
        """
        # Extract the name from the reference
        match = re.match(r'`bee:([^`]+)`', ref)
        if match:
            name = match.group(1)
