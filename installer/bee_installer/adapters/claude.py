"""
Claude Code adapter - native Bee format (passthrough).

Claude Code is the primary target platform for Bee, so this adapter
...
class ClaudeAdapter(PlatformAdapter):
    ...
    Claude Code uses Bee's native format, so transformations are minimal.
    ...
    def transform_skill(self, skill_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee skill for Claude Code.

        Since Claude Code uses Bee's native format, this is a passthrough.
    ...
    def transform_agent(self, agent_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee agent for Claude Code.
    ...
    def transform_command(self, command_content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        Transform a Bee command for Claude Code.
    ...
    def transform_hook(self, hook_content: str, metadata: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
        Transform a Bee hook for Claude Code.
    ...
    def get_component_mapping(self) -> Dict[str, Dict[str, str]]:
        ...
        Returns:
            Mapping of Bee components to Claude Code directories
        """
        return {
            "agents": {
                "target_dir": "agents",
    ...
    def get_terminology(self) -> Dict[str, str]:
        """
        Get Claude Code terminology (same as Bee).

        Returns:
            Identity mapping since Claude Code uses Bee terminology
        """
        return {
            "agent": "agent",
            "skill": "skill",
