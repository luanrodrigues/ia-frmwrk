"""
Bee Installer CLI - Multi-platform AI agent skill installer.

Usage:
    python -m bee_installer install [--platforms PLATFORMS] [--dry-run] [--force] [--verbose]
    python -m bee_installer update [--platforms PLATFORMS] [--dry-run] [--verbose]
    python -m bee_installer uninstall [--platforms PLATFORMS] [--dry-run] [--force]
    python -m bee_installer list [--platform PLATFORM]
    python -m bee_installer detect

Examples:
    # Install to all detected platforms
    python -m bee_installer install

    # Install to specific platforms
    python -m bee_installer install --platforms claude,cursor

    # Dry run to see what would be done
    python -m bee_installer install --dry-run --verbose

    # Update existing installation
    python -m bee_installer update

    # List installed components
    python -m bee_installer list --platform claude
...
from bee_installer import __version__
from bee_installer.adapters import SUPPORTED_PLATFORMS, list_platforms
...
def print_version() -> None:
    ...
    print(f"Bee Installer v{__version__}")
...
def find_bee_source() -> Optional[Path]:
    """
    Find the Bee source directory.

    Looks in these locations:
    1. Current directory (if it's a Bee repo)
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    # Find Bee source
    source_path = Path(args.source).expanduser() if args.source else find_bee_source()

    if not source_path or not source_path.exists():
        print("Error: Could not find Bee source directory.")
        print("Please specify with --source or run from Bee directory.")
        return 1

    print(f"Bee source: {source_path}")
...
def cmd_uninstall(args: argparse.Namespace) -> int:
    ...
    if not args.force:
        confirm = input("Are you sure? This will remove all Bee components. [y/N] ")
        if confirm.lower() != "y":
            print("Aborted.")
            return 0
...
def cmd_platforms(args: argparse.Namespace) -> int:
    ...
    for platform_info in list_platforms():
        ...
        if any(k != v for k, v in terminology.items()):
            print("    Terminology:")
            for bee_term, platform_term in terminology.items():
                if bee_term != platform_term:
                    print(f"      {bee_term} -> {platform_term}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="bee-installer",
        description="Bee multi-platform AI agent skill installer",
...
Examples:
  ...
  %(prog)s detect                     Detect installed platforms
    ...
    # Install command
    install_parser = subparsers.add_parser("install", help="Install Bee components")
    install_parser.add_argument(
        "--source", "-s",
        help="Path to Bee source directory"
    )
    install_parser.add_argument(
        "--platforms", "-p",
    ...
    # Update command
    update_parser = subparsers.add_parser("update", help="Update Bee components")
    update_parser.add_argument("--source", "-s", help="Path to Bee source directory")
    ...
    check_parser.add_argument("--source", "-s", help="Path to Bee source directory")
    check_parser.add_argument("--platforms", "-p", help="Comma-separated list of target platforms")

    # Sync command
    sync_parser = subparsers.add_parser("sync", help="Sync Bee components across platforms")
    sync_parser.add_argument("--source", "-s", help="Path to Bee source directory")
    ...
    # Uninstall command
    uninstall_parser = subparsers.add_parser("uninstall", help="Remove Bee components")
