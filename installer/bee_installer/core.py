"""
Core installation logic for Bee multi-platform installer.
...
from bee_installer.adapters import SUPPORTED_PLATFORMS, get_adapter
from bee_installer.utils.fs import safe_remove as _safe_remove
...
from bee_installer.utils.version import (
    get_bee_version as _get_bee_version,
)
...
get_bee_version = _get_bee_version
...
def _validate_marketplace_schema(
    ...
    Security: Ensures plugin sources don't escape the bee directory.

    Args:
        marketplace: Parsed marketplace JSON
        marketplace_path: Path to marketplace.json
        bee_path: Root path of the Bee installation
    ...
    bee_path_resolved = bee_path.resolve()
    ...
    for i, plugin in enumerate(plugins):
        ...
        # Security: Validate source path doesn't escape bee directory
        source = plugin["source"]
        if source.startswith("./"):
            source_path = bee_path / source[2:]
        else:
            source_path = bee_path / source

        try:
            source_resolved = source_path.resolve()
            source_resolved.relative_to(bee_path_resolved)
        except ValueError as e:
            raise ValueError(
                f"Invalid marketplace.json: plugin '{plugin.get('name')}' source path "
                f"'{source}' escapes bee directory (path traversal detected)"
            ) from e
...
def _verify_marketplace_integrity(bee_path: Path) -> None:
    ...
    """
    expected_hash = None
    checksum_file = bee_path / ".claude-plugin" / "marketplace.sha256"
    ...
    marketplace_path = bee_path / ".claude-plugin" / "marketplace.json"
    ...
    from bee_installer.utils.fs import get_file_hash
...
def discover_bee_components(bee_path: Path, plugin_names: Optional[List[str]] = None,
                             exclude_plugins: Optional[List[str]] = None) -> Dict[str, Dict[str, List[Path]]]:
    """
    Discover Bee components (skills, agents, commands) from a Bee installation.

    Args:
        bee_path: Path to Bee repository/installation
    ...
    if marketplace_path.exists():
        ...
        _validate_marketplace_schema(marketplace, marketplace_path, bee_path)
...
def install(
    ...
    """
    Install Bee components to one or more target platforms.

    Args:
        source_path: Path to the Bee repository/installation
    ...
    # Discover components
    components = discover_bee_components(
        source_path,
...
def update(
    ...
    """
    Update Bee components on target platforms.
...
def uninstall(
    ...
    """
    Remove Bee components from target platforms.
    ...
    for target in targets:
        ...
        for _component_type, config in component_mapping.items():
            ...
            try:
                ...
                if options.backup:
                    from bee_installer.utils.fs import backup_existing
...
def list_installed(platform: str) -> Dict[str, List[str]]:
    """
    List Bee components installed on a platform.
...
def check_updates(
    ...
    Args:
        source_path: Path to Bee source
...
def update_with_diff(
    ...
    """
    Update Bee components, only updating changed files.
    ...
    manifest = load_manifest()
    source_version = get_bee_version(source_path) or "0.0.0"
...
def sync_platforms(
    ...
    """
    Sync Bee components across multiple platforms.

    Ensures all target platforms have consistent Bee installations
...
def uninstall_with_manifest(
    ...
    """
    Remove Bee components using the install manifest for precision.

    Only removes files that were installed by Bee, preserving
    ...
    Returns:
        InstallResult with details about what was removed
    """
    from bee_installer.utils.fs import backup_existing, safe_remove
    options = options or InstallOptions()
    result = InstallResult(status=InstallStatus.SUCCESS, targets=[t.platform for t in targets])
