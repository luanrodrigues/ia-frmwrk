"""
Version management utilities for Bee installer.
...
class InstallManifest:
    """
    Manifest tracking installed Bee components.
...
def get_bee_version(bee_path: Path) -> Optional[str]:
    """
    Get the Bee version from a Bee installation.
    ...
    Args:
        bee_path: Path to Bee installation

    Returns:
        Version string or None if not found
    """
    bee_path = Path(bee_path).expanduser()
    ...
    package_json = bee_path / "package.json"
...
def get_installed_version(install_path: Path, platform: str) -> Optional[str]:
    """
    Get the installed Bee version for a platform.
...
def check_for_updates(
    ...
    # Get versions
    available = get_bee_version(source_path)
