...
class BaseTransformer(ABC):
    ...
    Transformers handle conversion of Bee component formats
    ...
    def transform_body_for_cline(self, body: str) -> str:
        ...
        # Transform bee: references to @ format
        result = re.sub(
            r'`bee:([^`]+)`',
...
class TerminologyTransformer(BaseTransformer):
    ...
    def __init__(self, terminology_map: Dict[str, str]):
        ...
        Args:
            terminology_map: Mapping from Bee terms to platform terms
        """
        super().__init__()
    ...
    def transform(self, content: str, context: TransformContext) -> TransformResult:
        ...
        for bee_term, platform_term in self.terminology_map.items():
            if bee_term != platform_term:
                # Case-sensitive replacements
                result = re.sub(rf'\b{bee_term}\b', platform_term, result)
                result = re.sub(rf'\b{bee_term.title()}\b', platform_term.title(), result)
                result = re.sub(rf'\b{bee_term.upper()}\b', platform_term.upper(), result)
