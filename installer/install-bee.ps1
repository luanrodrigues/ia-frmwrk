<#
.SYNOPSIS
    Bee Multi-Platform Installer (PowerShell)

.DESCRIPTION
    Installs Bee skills to Claude Code, Codex, Factory AI, Cursor, Cline, and/or OpenCode.
    Multi-platform installer wrapper for the Python-based Bee installer.
...
.NOTES
    ...
    Or run with bypass:
        powershell -ExecutionPolicy Bypass -File install-bee.ps1
...
.EXAMPLE
    .\install-bee.ps1 install --platforms claude
    Direct installation to Claude Code

.EXAMPLE
    .\install-bee.ps1 update
...
Write-Host "Bee Multi-Platform Installer" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
...
if ($args.Count -gt 0) {
    try {
        ...
        & $pythonExe @pythonArgs -m installer.bee_installer @args
        exit $LASTEXITCODE
    } catch {
...
Write-Host "Select platforms to install Bee:"
Write-Host ""
...
try {
    if ($dryRun -match "^[Yy]$") {
        ...
        Set-Location $RingRoot
        & $pythonExe @pythonArgs -m installer.bee_installer install --platforms $platformString --dry-run @extraArgs
    ...
    Set-Location $RingRoot
    & $pythonExe @pythonArgs -m installer.bee_installer install --platforms $platformString @extraArgs
    ...
    Write-Host ""
    Write-Host "Commands:"
    Write-Host "  .\installer\install-bee.ps1                    # Interactive install"
    Write-Host "  .\installer\install-bee.ps1 --platforms claude # Direct install"
    Write-Host "  .\installer\install-bee.ps1 update             # Update installation"
    Write-Host "  .\installer\install-bee.ps1 list               # List installed"
    Write-Host ""
} catch {
    Write-Host ""
    Write-Host "Error: $_" -ForegroundColor Red
