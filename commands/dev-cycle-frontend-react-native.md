...
description: Execute the React Native frontend development cycle for tasks in a markdown file
...
**Persistence:**
- **Stored:** `custom_prompt` field in `docs/bee:dev-cycle-frontend-react-native/current-cycle.json`
...
- MUST enforce 0 VoiceOver/TalkBack violations (Gate 2 Accessibility)
- MUST enforce 85% coverage (Gate 3 Unit Testing)
- MUST enforce performance thresholds (Gate 6 Performance)
...
- **State file**: `docs/bee:dev-cycle-frontend-react-native/current-cycle.json`
- **Feedback report**: `docs/dev-team/feedback/cycle-frontend-react-native-YYYY-MM-DD.md`
...
- UI Library Mode Detection (sindarian-rn vs fallback)
- Backend Handoff Loading
- State management schema
- React Native hooks patterns and conventions
- Expo / bare React Native detection (when applicable)
...
- **VoiceOver/TalkBack compliance** - 0 violations, no exceptions
- **Performance thresholds** - Bundle < 10% growth, Hermes enabled, FlashList for long lists
- **State persisted** - Can resume with `--resume` after any interruption
- **React Native only** - Functional components with hooks required; class components are a violation
- **Platform-aware** - If Platform.select() or .ios.tsx/.android.tsx patterns present, apply platform-specific conventions
