...
```
✅ Task(subagent_type="ring:backend-engineer-php", ...)
...
|------|-------------------|-----------------|
| 0 | `ring:backend-engineer-php` | Implements PHP/Laravel code, adds observability, runs TDD |
...
| 4 | `ring:backend-engineer-php` | PHP/Laravel standards compliance analysis |
...
|------------------|-------------------|
| `*.php` files | `ring:backend-engineer-php` |
...
| Test files (`*Test.php`, `*.spec.ts`) | `ring:qa-analyst` |
...
| "I already know PHP/TypeScript" | Knowing language ≠ knowing Ring standards. Agent has standards. | **DISPATCH specialist agent** |
