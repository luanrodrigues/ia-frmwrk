...
- unit_id exists
- language is valid (php|typescript|python)
...
Task:
  ...
  prompt: |
    ...
    <forbidden>
    - echo in PHP code
    - var_dump() in PHP code
    - dd() in PHP code
    - print_r() in PHP code
    - dump() in PHP code
    - error_log() in PHP code (use Monolog/Log facade instead)
    - console.log() in TypeScript (frontend context only - keep for BFF/frontend)
    - console.error() in TypeScript (frontend context only)
    - console.warn() in TypeScript (frontend context only)
    </forbidden>
    ...
    **MUST search for and report all occurrences of FORBIDDEN patterns:**
    ...
    |----------|-------------------|------------|
    | PHP | `echo` | `echo ` in *.php files (not in blade templates) |
    | PHP | `var_dump()` | `var_dump` in *.php files |
    | PHP | `dd()` | `dd(` in *.php files |
    | PHP | `print_r()` | `print_r` in *.php files |
    | PHP | `dump()` | `dump(` in *.php files |
    | PHP | `error_log()` | `error_log` in *.php files |
    ...
    **If any FORBIDDEN pattern found:**
    ...
    ### 1. Structured Logging (Monolog/Laravel Log)
    - [ ] Uses `Log::info()`, `Log::error()` etc. (Laravel Log facade) for PHP
    ...
    **PHP (OpenTelemetry PHP SDK):**
    ...
    $span = $tracer->spanBuilder('layer.operation')->startSpan();
    $scope = $span->activate();
    try { /* work */ } finally { $span->end(); $scope->detach(); }
    ```
    ...
    **TypeScript:**
    ...
    Count spans in:
    - Controllers: grep "spanBuilder" in *Controller*.php or *controller*.ts
    - Services: grep "spanBuilder" in *Service*.php or *service*.ts
    - Repositories: grep "spanBuilder" in *Repository*.php or *repository*.ts

    ### 3. Context Propagation
    For external calls, verify:
    - HTTP: OpenTelemetry HTTP middleware / Guzzle instrumentation (PHP) or equivalent
    - gRPC: OpenTelemetry gRPC instrumentation (PHP) or equivalent
    - Queues: OpenTelemetry queue propagation / Laravel Queue middleware (PHP) or equivalent
...
Task:
  ...
  prompt: |
    ...
    **Current:** [validation_state.sre_result.instrumentation_coverage]%
    ...
    ### If Logging Issues:
    - Replace echo/var_dump/dd/print_r with Laravel Log facade (Log::info(), etc.)
    - Add trace_id to log context
    - Use JSON format via Monolog

    ### If Instrumentation Coverage < 90%:
    - Add spans to all controllers: `$tracer->spanBuilder("controller.name")->startSpan()`
    - Add spans to all services: `$tracer->spanBuilder("service.domain.operation")->startSpan()`
    - Add spans to all repositories: `$tracer->spanBuilder("db.operation")->startSpan()`
    - Add `$span->end(); $scope->detach();` in finally blocks

    ### If Context Propagation Issues:
    - Add OpenTelemetry HTTP middleware for outgoing Guzzle/HTTP calls
    - Add OpenTelemetry gRPC instrumentation for outgoing gRPC calls
    - Add queue middleware for queue publishing propagation
...
| **CRITICAL** | echo/var_dump/dd instead of Log facade | FAIL | ❌ Return to Gate 0 |
