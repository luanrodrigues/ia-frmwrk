...
**HARD GATE:** MUST NOT recreate components that exist in `@lerianstudio/sindarian-vue`.
...
grep -r "export.*{ComponentName}" node_modules/@lerianstudio/sindarian-vue/
...
| Import paths correct | sindarian-vue → `@lerianstudio/sindarian-vue`, fallback → `@/components/ui/` |
...
<!--
    Fallback: not in @lerianstudio/sindarian-vue
