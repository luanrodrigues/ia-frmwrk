...
description: Analyze existing codebase against standards and execute refactoring through bee:dev-cycle
...
├── YES → Load skill: bee:dev-refactor
...
```
/bee:dev-refactor [path] [options] [prompt]
/bee:dev-refactor [prompt]
```
...
/bee:dev-refactor Check idempotency implementation against standards
...
/bee:dev-refactor src/domain Focus on error handling patterns
...
/bee:dev-refactor --standards docs/team-standards.md Prioritize security issues
```

## Workflow

**See skill `bee:dev-refactor` for the complete 13-step workflow with TodoWrite template.**

The skill defines all steps including: stack detection, bee:codebase-explorer dispatch, individual agent reports, finding mapping, and artifact generation.
...
|-----------|----------------|---------------------|
| **Architecture** | DDD patterns, layer separation, dependency direction, directory structure | `php.md` § Architecture |
| **Code Quality** | Naming conventions, error handling, forbidden practices, security | `php.md` § Error Handling |
| **Instrumentation** | Service method tracing, span naming, error classification, context propagation | `php.md` § Distributed Tracing |
| **Testing** | Coverage percentage, test patterns, naming, missing tests | `php.md` § Testing |
| **DevOps** | Dockerfile, docker-compose, env management, Helm charts | `php.md` § DevOps |
...
Example: `/bee:dev-refactor --critical-only` analyzes all issues but only executes fixes for Critical and High.

### Instrumentation Checklist (Quick Reference)

When analyzing services for instrumentation compliance, verify:

1. **Tracer initialization**: `$tracer = \OpenTelemetry\API\Globals::tracerProvider()->getTracer('app')` at service level
2. **Child span creation**: `$span = $tracer->spanBuilder('layer.entity.operation')->startSpan()` with proper naming
3. **Span cleanup**: `$span->end(); $scope->detach();` in finally blocks
4. **Error classification**:
   - Business errors → record exception on span, span stays OK
   - Technical errors → `$span->setStatus(StatusCode::STATUS_ERROR)` (span marked ERROR)
5. **Structured logging**: Use Laravel Log facade (`Log::info()`), not `echo` or `var_dump`

**Full details and code templates**: See `docs/standards/php.md` § "Distributed Tracing Architecture"

## Output

**Timestamp format:** `{timestamp}` = `YYYY-MM-DDTHH:MM:SS` (e.g., `2026-02-07T22:30:45`)

**Analysis Report** (`docs/bee:dev-refactor/{timestamp}/analysis-report.md`):
...
- Compatible with bee:dev-cycle execution
...
| Command | Description |
|---------|-------------|
| `/bee:dev-cycle` | Execute development cycle (used after analysis) |
| `/bee:pre-dev-feature` | Plan new features (use instead for greenfield) |
| `/bee:codereview` | Manual code review (bee:dev-cycle includes this) |
...
```
Use Skill tool: bee:dev-refactor
```
...
| `--analyze-only` | If provided, skip bee:dev-cycle execution |
...
**Before executing bee:dev-cycle, you MUST ask:**
...
See skill `bee:dev-refactor` for full details. Key rules:
...
- **Save artifacts** to `docs/bee:dev-refactor/{timestamp}/`
- **Get user approval** before executing bee:dev-cycle
- **Handoff**: `/bee:dev-cycle docs/bee:dev-refactor/{timestamp}/tasks.md`
