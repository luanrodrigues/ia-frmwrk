...
description: Execute the Vue.js frontend development cycle for tasks in a markdown file
argument-hint: "[tasks-file] [options] [prompt]"
---

Execute the Vue.js frontend development cycle for tasks in a markdown file or from a direct prompt.

## Usage

```
/bee:dev-cycle-frontend-vuejs [tasks-file] [options] [prompt]
/bee:dev-cycle-frontend-vuejs [prompt]
```
...
When a prompt is provided **without a tasks file**, bee:dev-cycle-frontend-vuejs enters prompt-only mode:
...
# Another example
/bee:dev-cycle-frontend-vuejs Add settings page with user profile, preferences, and notification controls
```
...
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md Focus on accessibility

# Tasks file with specific guidance
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md Use sindarian-ui DataTable for transaction list
```
...
**Persistence:**
- **Stored:** `custom_prompt` field in `docs/bee:dev-cycle-frontend-vuejs/current-cycle.json`
...
```bash
jq '.custom_prompt' docs/bee:dev-cycle-frontend-vuejs/current-cycle.json  # View
# Edit state file, then: /bee:dev-cycle-frontend-vuejs --resume
```

## Examples

```bash
# Direct prompt - implement Vue.js frontend feature
/bee:dev-cycle-frontend-vuejs Implement dashboard with transaction list and real-time charts

# Direct prompt - another example
/bee:dev-cycle-frontend-vuejs Add user settings page with profile, preferences, and notifications

# Execute all tasks from a file
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md

# Execute single task
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md --task FE-001

# Skip DevOps setup (infrastructure already exists)
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md --skip-gates devops

# Validate tasks without executing
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md --dry-run

# Resume interrupted cycle
/bee:dev-cycle-frontend-vuejs --resume

# Tasks file with additional context
/bee:dev-cycle-frontend-vuejs docs/tasks/sprint-001-frontend.md Prioritize accessibility and use DataTable from sindarian-ui
```
...
- **State file**: `docs/bee:dev-cycle-frontend-vuejs/current-cycle.json`
- **Feedback report**: `docs/dev-team/feedback/cycle-frontend-vuejs-YYYY-MM-DD.md`
...
```
Use Skill tool: bee:dev-cycle-frontend-vuejs
```

The skill file is located at: `skills/dev-cycle-frontend-vuejs/SKILL.md`
...
- Nuxt.js module and plugin detection (when applicable)
...
AskUserQuestion:
  questions:
    - question: "Select execution mode for this Vue.js frontend cycle"
      header: "Mode"
      options:
...
See skill `bee:dev-cycle-frontend-vuejs` for full details. Key rules:
...
- **Vue 3 only** - Composition API with `<script setup>` syntax required; Options API is a violation
- **Nuxt-aware** - If `nuxt.config.ts` is present, apply Nuxt-specific conventions (auto-imports, server routes, layouts)
