...
- Refactoring work (use `/bee:dev-refactor` first)
...
See `bee:dev-cycle` skill's "Custom Prompt Injection" section for full validation logic.
...
1. **Task file or prompt**: Either a markdown file generated by `/pre-dev-*` or `/bee:dev-refactor`, or a direct prompt
...
- **State file**: `docs/bee:dev-cycle/current-cycle.json` (feature) or `docs/bee:dev-refactor/current-cycle.json` (refactor)
