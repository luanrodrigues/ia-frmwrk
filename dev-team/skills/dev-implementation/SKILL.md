...
verification:
  automated:
    - command: "php artisan route:list 2>&1 | grep -c 'ERROR'"
      description: "PHP code loads successfully"
      success_pattern: "^0$"
...
examples:
  - name: "PHP backend implementation"
    input:
      unit_id: "task-001"
    ...
    expected_output: |
      ...
      **Agent:** bee:backend-engineer-php
      ...
      | Phase | Status | Output |
      |-------|--------|--------|
      | RED | ✅ | FAIL: UserAuthTest - expected token, got null |
      | GREEN | ✅ | PASS: UserAuthTest (0.003s) |

      ## Files Changed
      | File | Action | Lines |
      |------|--------|-------|
      | app/Http/Controllers/AuthController.php | Created | +85 |
...
Task:
  ...
  prompt: |
    ...
    - Use Pest datasets/PHPUnit data providers (PHP) or describe/it blocks (TS)
...
Task:
  ...
  prompt: |
    ...
    **⛔ HARD GATE: Agent MUST WebFetch standards file BEFORE writing any code.**
    ...
    |----------|----------------|-------------------------------|
    | **PHP** | `php.md` | "Telemetry & Observability (MANDATORY)", "OpenTelemetry PHP SDK", "Context Propagation", "Anti-Patterns" |
    ...
    **Agent MUST WebFetch standards and check Anti-Patterns table. Violations = REJECTED.**
    ...
    - **PHP:** `php.md` → "Anti-Patterns" table - MUST check all rows
...
| "Using raw OTel is fine" | Laravel/OpenTelemetry wrappers are MANDATORY for consistency | **Use OpenTelemetry PHP SDK with Laravel integration** |
| "response()->json() is enough" | Direct responses break standardization | **Use API Resource classes and consistent response helpers** |
