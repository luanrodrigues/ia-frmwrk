...
**HARD GATE:** MUST NOT recreate components that exist in `@luanrodrigues/sindarian-ui`.
...
grep -r "export.*{ComponentName}" node_modules/@luanrodrigues/sindarian-ui/
...
| Import paths correct | sindarian-ui → `@luanrodrigues/sindarian-ui`, fallback → `@/components/ui/` |
