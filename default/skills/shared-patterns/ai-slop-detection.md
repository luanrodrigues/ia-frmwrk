...
|-------|-------------------|----------|------------------|
| **Phantom Dependencies** | Package names that don't exist on npm/PyPI/Packagist | CRITICAL | AI hallucinates plausible names from training data |
...
| **Stateless class when functions would suffice** | AI creates class with no properties (PHP-specific) | Does the class hold any state? |
...
### Stateless Class Pattern (PHP)

```php
// ❌ OVER-ENGINEERED: Class with no properties
class ScopeMatcher
{
    public function match(string $scope, string $pattern): bool
    {
        return str_starts_with($scope, $pattern);
    }
}
...
// ✅ SIMPLE: Static method or plain function
function matchScope(string $scope, string $pattern): bool
{
    return str_starts_with($scope, $pattern);
}
...
**When stateless classes ARE justified:**
- Implementing an interface (e.g., `Middleware`, `Rule`)
