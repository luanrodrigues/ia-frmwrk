...
`<lint_command> 2>&1 | tee /tmp/bee:lint-output.txt && echo "EXIT_CODE: $?"`
...
`/bee:lint` → Run lint → 16 issues in 3 areas → Analyze streams (API: 5, Services: 8, Utils: 3) → Dispatch 3 parallel agents → All complete → Re-run lint → ✅ All pass.
