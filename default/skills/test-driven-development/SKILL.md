...
compliance_rules:
  ...
  - id: "test_must_fail_first"
    ...
    command: "npm test 2>&1 || pytest 2>&1 || php artisan test 2>&1"
...
prerequisites:
  - name: "test_framework_installed"
    check: "npm list jest 2>/dev/null || npm list vitest 2>/dev/null || which pytest 2>/dev/null || php artisan test --version 2>/dev/null"
    failure_message: "No test framework found. Install jest/vitest (JS), pytest (Python), or use PHPUnit/Pest (PHP)."
    severity: "blocking"

  - name: "can_run_tests"
    check: "npm test -- --version 2>/dev/null || pytest --version 2>/dev/null || ./vendor/bin/phpunit --version 2>/dev/null"
...
composition:
  works_well_with:
    ...
    - skill: "bee:verification-before-completion"
