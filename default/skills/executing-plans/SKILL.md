...
skip_when: |
  - Same session with independent tasks → use bee:subagent-driven-development
  - No plan exists → use bee:writing-plans first
  - Plan needs revision → use bee:brainstorming first

sequence:
  after: [bee:writing-plans, bee:pre-dev-task-breakdown]

related:
  similar: [bee:subagent-driven-development]
---
...
**Announce at start:** "I'm using the bee:executing-plans skill to implement this plan."
...
**Agent Selection:** Backend Go → `bee:backend-engineer-golang` | Backend TS → `bee:backend-engineer-typescript` | Frontend → `bee:frontend-bff-engineer-typescript` | Infra → `bee:devops-engineer` | Testing → `bee:qa-analyst` | Reliability → `bee:sre`
...
**After each batch, REQUIRED:** Use bee:requesting-code-review (all 6 reviewers in parallel)
