...
2. **A skill has its own specialized agents** - Some skills (e.g., `bee:dev-refactor`) define their own agents that MUST be used instead of Explore/Plan/general-purpose. When a skill specifies "OVERRIDE" or "FORBIDDEN agents", follow the skill's agent requirements, not the defaults above.
...
**Bee:** `bee:code-reviewer`, `bee:business-logic-reviewer`, `bee:security-reviewer`, `bee:write-plan`.
...
| Implementation plan document | bee:write-plan |
...
- **Always announce meta-skills:** brainstorming, bee:writing-plans, systematic-debugging (methodology change)
