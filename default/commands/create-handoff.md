...
# /bee:create-handoff Command

Creates a comprehensive handoff document that captures the current session's context, progress, decisions, and next steps. After creating the handoff, simply run `/clear` - the handoff will be **automatically loaded** in the new session (no need to run `/bee:resume-handoff` manually).

## Usage

```
/bee:create-handoff [session-name] [description]
```
...
1. **Create handoff**: `/bee:create-handoff auth-refactor "OAuth integration complete"`
2. **Clear context**: Run `/clear` to reset conversation
3. **Auto-resume**: The handoff is automatically loaded in the new session

> **Note:** If more than 1 hour passes between creating the handoff and running `/clear`, you'll be asked whether to resume instead of auto-loading. For manual resume of older handoffs, use `/bee:resume-handoff <path>`.
...
### With Session Name
```
User: /bee:create-handoff auth-refactor "OAuth provider integration"
