...
```bash
/bee:explore-codebase account creation
```
...
```bash
/bee:explore-codebase transaction processing
```
...
```bash
/bee:explore-codebase authentication system
```
...
```bash
/bee:explore-codebase transaction processing in payment service only
```

### Deep Exploration
Request comprehensive analysis:
```bash
/bee:explore-codebase authentication (include all integrations and edge cases)
```
...
```bash
# Before refactoring
/bee:explore-codebase user management
...
```
Ready to create implementation plan? Use /bee:write-plan
```

**If you're setting up workspace:**
```
Ready for isolated workspace? Use /bee:worktree
```
...
**If you're designing:**
```
Ready to refine design? Use /bee:brainstorm
```
...
/bee:explore-codebase payment processing

# 2. Review synthesis document (architecture + implementation)

# 3. Based on findings, plan your changes
/bee:write-plan add-refund-support

# 4. Set up isolated workspace
/bee:worktree feature/add-refund-support

# 5. Execute the plan
/bee:execute-plan <plan-file>
```
